...
<img src="./public/images/main_website_aws.png" alt="Cloudiverse App Dashboard" width="100%" />
...
## 📁 Repository Strategy: Showcase & Core Code Separation

To showcase this enterprise-grade design professionally on your resume/portfolio **without exposing database configurations or proprietary backend controllers**, the project is organized into two repositories:

### 1. The Public Showcase Repo (This Repository)
This repository contains **only** system design specifications, guides, and architectural whitepapers.
* **Included Files**:
  * `README.md` (This file!)
  * `Project_Architecture_Report.md` (The complete system design whitepaper mapping SAA-C03 domains)
  * `AWS_Docker_ECR_ECS_Integration_Guide.md` (Docker & ECS deployment manual)
  * An `images/` directory containing screenshots of the app, logs, and system console.
* **DO NOT Publish**: Raw application source files (`backend/`, `frontend/`), configuration files containing database pool credentials, or raw CI/CD workflow files.

### 2. The Private Repository (Your Core Codebase)
Stores the actual codebase powering the application:
* `backend/` — Express backend controllers, database migrations, LLM prompt templates, and middleware.
* `frontend/` — React frontend context, canvas layout components, and custom CSS.
* `.github/workflows/deploy.yml` — GitHub Actions automated build-and-deploy pipeline.
...
* **Network Isolation**: The ECS tasks and database live inside isolated private subnets with no public IPs. Ingress is allowed *only* from the ALB, and egress is restricted via NAT Gateways.
* **Database Access Restriction**: The RDS PostgreSQL database is placed in the database subnet and accepts traffic *exclusively* from Fargate containers on port `5432`.
...
<img src="./public/images/actions_cicd.png" alt="GitHub Actions Deployment Workflow Running" width="100%" />
